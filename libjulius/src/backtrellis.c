 ...
 */
void
set_terminal_words(Recog *recog)
{
  LOGPROB maxscore;
  int i,t;
  BACKTRELLIS *bt;
  ...
  for(i=0;i<bt->num[t];i++) {
    if (maxscore < (bt->rw[t][i])->backscore) {
      ...
      recog->sp_break_2_begin_word = (bt->rw[t][i])->wid;
    }
  }
  maxscore = LOG_ZERO;
  ...
  for(i=0;i<bt->num[t];i++) {
    if (maxscore < (bt->rw[t][i])->backscore) {
      maxscore = (bt->rw[t][i])->backscore;
      recog->sp_break_2_end_word = (bt->rw[t][i])->wid;
    }
  }
#ifdef SP_BREAK_DEBUG
  jlog("DEBUG: 2nd pass begin word: %s\n",
	 (recog->sp_break_2_begin_word == WORD_INVALID) ? "WORD_INVALID" : winfo->wname[recog->sp_break_2_begin_word]);
  jlog("DEBUG: 2nd pass end word: %s\n",
	 (recog->sp_break_2_end_word == WORD_INVALID) ? "WORD_INVALID" : winfo->wname[recog->sp_break_2_end_word]);
#endif
}
