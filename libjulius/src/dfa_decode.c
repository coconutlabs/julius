 ...
 */
int
dfa_firstwords(NEXTWORD **nw, int peseqlen, int maxnw, Recog *recog)
{
  DFA_INFO *dfa;
  DFA_ARC *arc;
  MULTIGRAM *m;
	...
	if ((dfa->st[s].status & INITIAL_S) != 0) { /* from initial state */
	  for (arc = dfa->st[s].arc; arc; arc = arc->next) {	/* for all arc */
	    ...
	    for (iw=0;iw<dfa->term.wnum[cate];iw++) {
	      ...
	      nw[num]->lscore = recog->jconf->lm.penalty2;
	      num++;
 ...
 */
int
dfa_nextwords(NODE *hypo, NEXTWORD **nw, int maxnw, Recog *recog)
{
  DFA_INFO *dfa;
  DFA_ARC *arc, *arc2;
 ...
 */
boolean
dfa_acceptable(NODE *hypo, Recog *recog)
{
  if (recog->model->dfa->st[hypo->state].status & ACCEPT_S) {
    return TRUE;
  } else {
    return FALSE;
  }
 ...
 */
boolean
dfa_look_around(NEXTWORD *nword, NODE *hypo, Recog *recog)
{
  int t,tm;
  int i;
  WORD_ID w;
